    ...
    if request.method == 'POST':
        try:
            ...
            if not email.endswith('@innovasolutions.com'):
                return "Please use your @innovasolutions.com email address", 400

            if personal_email and not re.match(r'^[^@]+@[^@]+\.[^@]+$', personal_email):
                return "Invalid personal email format", 400
            ...
            try:
                ...
                print(f"Warning: Could not process image for face recognition: {str(e)}")
                encodings = None
            ...
            with get_db() as conn:
                ...
                cursor.execute('''
                    INSERT INTO users (emp_id, full_name, email, personal_email, image_path, face_encoding, password, department, position)
                    VALUES (?, ?, ?, ?, ?, ?, ?, '', '')
                ''', (
                    emp_id,
                    full_name,
                    ...
                    personal_email,
                    image_path,
                    str(encodings[0].tolist()) if encodings else '[]',
                    hashed_password
                ))
                conn.commit()
...
def auto_signin():
    ...
    try:
        ...
        if not matched_user:
            return jsonify({'success': False, 'message': 'User not recognized'}), 404

        if best_match_score < 0.6:
            return jsonify({'success': False, 'message': 'Recognition confidence too low'}), 400
        ...
        return jsonify({'success': False, 'message': f"An error occurred during sign-in: {str(e)}"}), 500
